...
像 **Claude Code**、**OpenAI Codex** 和 Google **Antigravity (AGY)** 这类终端智能体，本质上并不只是写代码的工具。它们拥有宿主操作系统的完整权限：跑 Shell 脚本、查系统日志、管理 Docker 容器、运行 Python 处理数据，以及调用各类自定义技能（Skills）。

但它们有一个共同限制：**你必须坐在电脑前守着终端窗口**。

**PocketAgent** 是一个跑在本地的轻量级网关，把这三个 CLI 统一接入到 **Telegram** 与 **Discord**。无论是在手机、平板、远程笔记本还是桌面端聊天软件里，随时随地发条消息，本地电脑就能直接在后台执行任务并把结果实时同步给你，不用再死守着终端窗口，也无需配置复杂的 SSH 端口映射或远程桌面。
   ...
   Claude Code       OpenAI Codex    Google Antigravity
      (CLI)             (CLI)              (AGY)
        ...
        └─────────────────┼─────────────────┘
                          ▼
                你的本地电脑与操作系统环境
   (Bash/Zsh • Python • Git • Docker • 文件系统 • 自定义技能)
```
...
- **多 Bot 与多渠道支持**：单实例支持同时挂载多个 Telegram 和 Discord Bot。每个 Bot 可单独指定默认引擎、模型、推理强度（effort）以及专属 System Prompt（`SOUL.md`）。
...
| 场景 | 发送消息 / 指令 | 本地机器实际执行 |
| :--- | :--- | :--- |
| **远程运维** | `看看生产环境 nginx 为什么一直 502` | CLI 执行 `docker logs` 并排查端口或配置，把诊断分析返回到聊天窗口 |
