...
| 场景 | 发送消息 / 指令 | 本地机器实际执行 |
| :--- | :--- | :--- |
| **文档与图片解析** | 发送 PDF 论文 / 合同 / 账单截图：`提炼核心论点并列出费用汇总` | CLI 调取多模态能力或本地脚本解析内容，结构化输出要点到聊天窗口 |
| **实时资讯与检索** | `搜索今天关于 AI 领域的最新重磅动态，整理一份早报` | CLI 实时联网检索权威信源并交叉验证，生成精炼摘要即时推送 |
| **创意与图像生成** | `让 Codex 生成一张未来赛博朋克风格的海报，保存到桌面` | CLI 调取图像生成工具绘制并保存，同时将成品图片发回聊天窗口 |
| **随时改代码与测试** | `把 auth.ts 里的 token 过期时间改长一点，跑下 vitest` | CLI 自动定位修改文件并运行测试套件，返回测试结果与 Git Diff |
| **日常数据与文件整理** | `统计一下 Downloads 目录下按体积最大的前 10 个文件` | CLI 运行本地 Shell/Python 脚本快速盘点，返回直观列表与清理建议 |
| **跨模型协作** | 用 Claude 设计架构后，输入 `/engine` 选 `agy` | 自动携带上下文切到 Antigravity，利用海量上下文进行全工程实现 |
